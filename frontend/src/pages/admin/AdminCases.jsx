...
export function AdminCases() {
  ...
  const fetchFirstPage = useCallback(async () => {
    ...
    try {
      const data = await listAllCases({ limit: 25 });
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const data = await listAllCases({ cursor: pageInfo.nextCursor, limit: 25 });
      if (generation !== requestGeneration.current) return;
                    ...
                    <tbody>
                      {cases.map((c) => (
                        ...
                        >
                          ...
                          <td style={{ padding: 'var(--space-4)' }}>{formatDate(c.updatedAt)}</td>
                  ...
                  {cases.map((c) => (
                    ...
                    >
                      <div className="mobile-list-card-header">
                        <span className="mobile-case-code" style={{ fontWeight: 'var(--font-weight-bold)', fontSize: 'var(--font-size-md)' }}>{c.caseCode}</span>
                        <span className="mobile-case-date" style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-tertiary)' }}>{formatDate(c.updatedAt)}</span>
                      </div>
